...
def obj_func(xs):
    val = pd.Series(xs).value_counts().index[0]
    return int(val)


class ClassificationDataset(torch.utils.data.Dataset):
    def __init__(self, total_count, length, embedding=False):
        self.data = torch.randint(0, 10, (total_count, length)) if embedding else torch.randint(0, 10, (total_count, length, 1)).type(torch.FloatTensor)
...
class Datasets:
    def __init__(self, batch_size, total_count=1000, length=15, embedding=False):
        self.train_loader = torch.utils.data.DataLoader(ClassificationDataset(total_count, length, embedding),
                                                        batch_size=batch_size, shuffle=True)
        self.test_loader = torch.utils.data.DataLoader(ClassificationDataset(total_count, length, embedding),
...
class RNNClassifier2(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        o1 = torch.squeeze(x)
        o2 = self.embedding(o1)
        o3 = self.lstm(o2)
        o4 = torch.squeeze(o3[1][0])
        o5 = self.fc(o4)
